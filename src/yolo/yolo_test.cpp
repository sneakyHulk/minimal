...
int main() {
	...
	try {
		std::string model_path = std::filesystem::path(CMAKE_SOURCE_DIR) / std::filesystem::path("yolov8n.torchscript");
		torch::jit::script::Module yolo_model;
		yolo_model = torch::jit::load(model_path, device);
		yolo_model.eval();

		// Load image and preprocess
		cv::Mat image = cv::imread(std::filesystem::path(CMAKE_SOURCE_DIR) / std::filesystem::path("s110_w_cam_8_images/1690366213555.jpg"));
		cv::Mat input_image;
